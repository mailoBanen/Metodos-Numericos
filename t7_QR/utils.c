...
double **old_read_matrix(char *source, int n){
...
double *old_read_vec(char *source, int n){
...
void display_sys_eq(double **A, double *b, int n){
...
void err_fact(double **A, double **L, double **U, int n){

    double **R = create_matrix(n);
    multiply(L, U, R, n);

    double max_val = fabs(A[0][0] - R[0][0]);
    double curr_val;
    ...
    printf("El error maximo es de: %e\n", max_val);
    free_matrix(R, n);
}
...
void save_matrix(const char *filename, int n, double **M){
...
void save_vector(const char *filename, int n, double *b) {
    FILE *f = fopen(filename, "w");
    if (!f) {
        perror("Could not open file");
        exit(1);
